...
docker kill cliDealer cliManufacturer logspout || true
./network.sh down
./network.sh up createChannel -ca -s couchdb

# Copy the connection profiles so they are in the correct organizations.
cp "${DIR}/../test-network/organizations/peerOrganizations/org1.example.com/connection-org1.yaml" "${DIR}/network-participants/dealer/gateway/"
cp "${DIR}/../test-network/organizations/peerOrganizations/org2.example.com/connection-org2.yaml" "${DIR}/network-participants/manufacturer/gateway/"
...
echo "./network-participants/manufacturer/configuration/cli/monitordocker.sh net_test"
